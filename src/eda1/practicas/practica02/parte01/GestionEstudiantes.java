...
public class GestionEstudiantes implements Iterable<Par<Integer, ArrayList<Par<String, String>>>> {
	...
	public String getEquipoDocenteEstudiante(String estudianteId) {
		...
		for (AsignaturaNotas asignaturaNotas : estudiante) {
			Asignatura asignatura = asignaturaNotas.getAsignatura();
			String equipoDocente = asignatura.toStringDocentes(null);
			String[] docentes = equipoDocente.replace("[", "").replace("]", "").split(", "); 
			/* Elimino los corchetes que añade toStringDocentes(null);
			   Divido la cadena de equipoDocente en docentes individuales */ 
			for (String docente : docentes) {
				if (!result.contains(docente)) {
					result.add(docente);
				}
			}
		}
	...
	public Iterator<Par<Integer, ArrayList<Par<String, String>>>> iterator() {
     ...
     /*Como nos piden las notas medias de cada asignatura de su cuatrimestre que les corresponden
	   hacemos un for de todos los cuatrimestres que haya, en nuestro caso 7	*/
		...
		/*Empezamos "armando" nuestro Par, donde primero creamos la parte del Value que nos interesa*/
			ArrayList<Par<String, String>> asignaturasPorCuatrimestre = new ArrayList<>();
			for (Asignatura asignatura : asignaturasOfertadas) {//Iteramos sobre todas las asignaturas que hay
				int igual = asignatura.getCuatrimestre(); // Guardamos en una variable el Nº Cuatrimestre que pertenece esa asignatura 
				String nota = getNotaMediaAsignatura(asignatura.getAsignaturaId());//Usamos el metodo getNotaMediaAsignatura para consegir la nota media de esa asignatura
				if (igual != cuatrimestre)continue;  // Si la asignatura no es igual al cuatrimestre que nos interesa pasa al siguiente asignatura
				asignaturasPorCuatrimestre.add(new Par<>(asignatura.getAsignaturaId(), nota));// Añadimos los datos al Par<String, String>
			}
			result.add(new Par<>(cuatrimestre, asignaturasPorCuatrimestre));// Añadimos los datos al Par<Integer, ArrayList<Par<String, String>>>
		}

		return result.iterator(); //Devolvemos los datos
	}
