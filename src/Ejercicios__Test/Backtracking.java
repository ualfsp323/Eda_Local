...
public class Backtracking {
	public Pair<Integer, Integer> search(final List<Pair<Integer, Integer>> range, final Heuristics heuristics) {
	...
	private Pair<Integer, Integer> search(final int[] state, final List<Pair<Integer, Integer>> range,
			final Pair<Integer, Integer> optimal, final int branch, final Heuristics heuristics) {
		...
		IntStream.range(rangeFrom, rangeTo).forEach(cursor -> {
			...
			if (heuristics.getPruning().test(state)) {
				if (branch < state.length - 1) {
					optimal.update(this.search(state.clone(), range, optimal, branch + 1, heuristics));
