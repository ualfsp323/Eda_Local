...
import eda1.practicas.practica02.parte01.Asignatura;
import eda1.practicas.practica02.parte01.Estudiante;
...
public class Practica03Parte03Test {
	...
	public void test02() {
		Node.initializeNumNodes();
		Node<Cliente> node01 = new Node<>(new Cliente2("pepe"), new Cliente2("angela"));
		Node<Cliente> node02 = new Node<>(new Cliente2("pepe"), new Cliente2("angela"));
		Node<Cliente> node03 = new Node<>(new Cliente2("angela"));
		Node<Cliente> node04 = new Node<>(new Cliente2("antonio"), new Cliente2("zacarias"));
		Node<Cliente> node05 = new Node<>(new Cliente2("zacarias"), new Cliente2("emilio"));
	...
	@Order(4)
	public void test04() {
		// Crear nodos para representar ciudades y sus distancias
		Node<String> nodeA = new Node<>("CityA");
		Node<String> nodeB = new Node<>("CityB");
		Node<String> nodeC = new Node<>("CityC");
		Node<String> nodeD = new Node<>("CityD");

		// Crear una colección de nodos para representar el grafo
		NodeCollection<String> graph = new NodeCollection<>();

		// Establecer conexiones entre ciudades (nodos)
		graph.add(nodeA, nodeB);
		graph.add(nodeB, nodeC);
		graph.add(nodeC, nodeD);
		graph.add(nodeD, nodeA);
		graph.add(nodeA, nodeC);


		// Verificar el tamaño de la colección y las conexiones entre nodos
		assertEquals(4, graph.size());
		assertTrue(graph.getNeighbours(nodeA).contains(nodeB));
		assertTrue(graph.getNeighbours(nodeB).contains(nodeC));
		assertTrue(graph.getNeighbours(nodeC).contains(nodeD));
		assertTrue(graph.getNeighbours(nodeD).contains(nodeA));

		// Visualizar el grafo ordenado alfabéticamente por ciudades
		String orderedGraph = graph.toStringOrdered(Comparator.naturalOrder());
		assertEquals("{[CityA]=[[CityB], [CityC]], [CityB]=[[CityC]], [CityC]=[[CityD]], [CityD]=[[CityA]]}",orderedGraph);


		// Limpiar el grafo y verificar que está vacío
		graph.clear();
